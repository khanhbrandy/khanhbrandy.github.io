...
// Read-only item
public readonly decimal tax_owed;
...
int sum = numbers.Sum(); // Sum all elements
...
listofstudents.Add(new Student("Jaspreet", "Sahiwal", 01, new List<int> { 100, 99, 98 }));
listofstudents.Add(new Student("Ngoc-Khanh", "Trinh", 02, new List<int> { 91, 90, 89 }));
var mylist = (from student in listofstudents
                  //where student.scores.Sum() > 280
...
{
    const int MAX = 100;
    void taxcal(double salary); // methods are public by default
    void deductions(double salary);
}

/* Derive from Inferface */
public class Sale : IComparable<Sale>
{
    public int CompareTo(Sale o) //// Any class which extends the interface  has to explicity  mention the methods as public when overriding 
    {
        if (this.tax_owed > o.tax_owed)
        {
            return 1;
        }
        else if (this.tax_owed == o.tax_owed)
        {
            return 0;
        }
        else
...
// this
public SalesTransaction(string salesperson_name, double sales_amount):this(salesperson_name, sales_amount, 0) { }
// base
public SpecialCookieOrder(int Order_number, string Name, string Type, int No_dozens) :base(Order_number, Name, Type, No_dozens) { }

/* Overloaded + operator */
public static SalesTransaction operator +(SalesTransaction t1, SalesTransaction t2)
{
    SalesTransaction new_t = new SalesTransaction();
    new_t.sales_amount = t1.sales_amount + t2.sales_amount;
    return new_t;
...
public override bool Equals(object obj)
{
    bool check = true;
    Job a_job = (Job)obj;
    if (this.Job_number != a_job.Job_number)
    {
        check = false;
    }
    return check;
...
{
    string message = $"Job number {Job_number} of customer {Customer_name}, the duty is {Job_description}, completed in {Estimated_hours} hours, with a price of {price}";
    return message;
}

public int CallOverriddenGetSleepAmt()
{
    return base.GetSleepAmt(); // Calls method defined in base class
}